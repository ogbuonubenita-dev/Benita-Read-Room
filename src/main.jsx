...
const buy = async (b) => {
 ...
 const BookCard=({b})=><article className="card"><Cover b={b}/><h3>{b.title}</h3><p>{b.desc}</p><div className="row"><b>₦{b.price.toLocaleString("en-NG")}</b><button className="pill dark" onClick={()=>buy(b)}>Buy book</button></div></article>;
